  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-2xl border border-ink-200 bg-white p-6 shadow-card">
        <h2 className="text-h4">{SUPPORT_SIDEBAR.heading}</h2>

        <ul className="mt-5 flex flex-col gap-5">
          {SUPPORT_SIDEBAR.items.map(({ icon: Icon, title, description, link, cta }) => (
            <li key={title} className="flex items-start gap-3.5">
              <span className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-full bg-brand-50">
                <Icon aria-hidden="true" className="size-4.5 text-brand-600" />
              </span>
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[0.8125rem] text-ink-600">{description}</p>
                {cta ? (
                  <SiteLink
                    href={cta.href}
      ...
      <div className="rounded-2xl border border-ink-200 bg-white p-6 shadow-card">
        <div className="flex items-start gap-3.5">
          <span className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-full bg-brand-50">
            <ShieldAlert aria-hidden="true" className="size-4.5 text-brand-600" />
          </span>
          <div className="min-w-0">
            ...
            <p className="mt-1 text-[0.8125rem] text-ink-600">
              {SUPPORT_SIDEBAR.securityConcern.note}
            </p>
          </div>
        </div>
      </div>
