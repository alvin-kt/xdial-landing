...
function CallStatusCard() {
  return (
    <div className="w-full max-w-[19rem] rounded-2xl border border-white/10 bg-navy-900/80 p-4 shadow-card-lg backdrop-blur-sm sm:p-5">
      <div className="flex items-center gap-3">
        <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-brand-600">
          <Phone aria-hidden="true" className="size-4 text-white" />
        </span>
      ...
      <ul className="mt-3 flex flex-col gap-2">
        {BOOK_DEMO_CALL.steps.map((step) => (
          <li key={step.label} className="flex items-center justify-between gap-3 text-sm">
            ...
            {step.done ? (
              <span className="flex size-4.5 shrink-0 items-center justify-center rounded-full bg-brand-500 text-white">
                <Check aria-hidden="true" className="size-3" strokeWidth={3} />
              </span>
            ) : (
              <span
                aria-hidden="true"
                className="size-4.5 shrink-0 rounded-full border-2 border-white/25"
              />
            )}
          </li>
        ))}
    ...
    <div className="flex flex-col">
      ...
      <ul className="mt-8 flex flex-col gap-5">
        {BOOK_DEMO_HERO.checklist.map(({ title, description }) => (
          <li key={title} className="flex items-start gap-3.5">
            <span className="mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-full bg-brand-600">
              <Check aria-hidden="true" className="size-3.5 text-white" strokeWidth={3} />
            </span>
            <span className="min-w-0">
              <span className="block text-base font-semibold text-white">{title}</span>
              <span className="mt-0.5 block text-sm text-frost-500">{description}</span>
            </span>
          </li>
        ))}
      </ul>
      ...
      <div className="mt-10 flex flex-col items-center gap-6 sm:flex-row sm:items-end">
        <img
          ...
          className="h-auto w-[11rem] shrink-0 select-none sm:w-[13rem]"
          loading="lazy"
          decoding="async"
          draggable={false}
        />
